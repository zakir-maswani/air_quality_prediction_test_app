...
# PROFESSIONAL STYLING - LIGHT ORANGE THEME
st.markdown("""
<style>
    /* Main Container */
    .main {
    ...
    /* Card Styling - WHITE WITH LIGHT ORANGE ACCENT */
    .metric-card {
        background: #ffffff;
        border-radius: 12px;
        padding: 1.5rem;
        box-shadow: 0 2px 8px rgba(253, 191, 96, 0.15);
        border-left: 5px solid #fdbf60;
        margin-bottom: 1rem;
    }
    ...
    .glass-card {
        ...
        border-radius: 12px;
        padding: 1.8rem;
        box-shadow: 0 4px 15px rgba(253, 191, 96, 0.12);
    ...
    /* Button Styling - LIGHT ORANGE */
    .stButton > button {
        background: linear-gradient(90deg, #fdbf60 0%, #fed7aa 100%);
        color: #5a3a1a;
        border: none;
        border-radius: 8px;
    ...
    .stButton > button:hover {
        ...
        box-shadow: 0 6px 20px rgba(253, 191, 96, 0.4);
        background: linear-gradient(90deg, #fed7aa 0%, #fdbf60 100%);
    }
    ...
    [data-testid="stSidebar"] {
        background: linear-gradient(180deg, #fef3e2 0%, #fde8d0 100%);
    }
...
def load_all_data():
    ...
    with st.spinner("Loading DEFRA UK Air Quality Data..."):
        ...
        st.success("Data loaded for {} cities".format(len(all_data)))
...
def show_dashboard():
    ...
    with col1:
        st.metric("PM2.5", "{:.1f} µg/m³".format(latest['PM2.5']), delta=status)
    
    with col2:
        st.metric("PM10", "{:.1f} µg/m³".format(latest['PM10']))
    
    with col3:
        st.metric("Temperature", "{:.1f}°C".format(latest['Temperature']))
    
    with col4:
        st.metric("Humidity", "{:.0f}%".format(latest['Humidity']))
    
    # Alert box
    st.markdown("""
    <div class="alert-box alert-warning">
        <strong>Air Quality Status: {}</strong><br/>
        {}
    </div>
    """.format(status, get_health_recommendations(status)), unsafe_allow_html=True)
    ...
    with col1:
        ...
        fig.add_trace(go.Scatter(
            ...
            mode='lines+markers',
            name='PM2.5',
            line=dict(color='#fdbf60', width=3),
            fill='tozeroy',
            fillcolor='rgba(253, 191, 96, 0.2)'
        ))
        fig.update_layout(
    ...
    with col2:
        ...
        fig = go.Figure(data=[go.Pie(
            labels=pollutants,
            values=values,
            marker=dict(colors=['#fed7aa', '#fdbf60', '#f59e0b', '#d97706'])
        )])
        fig.update_layout(
    ...
    if st.session_state.models_trained:
        ...
        for model, metrics in st.session_state.model_performance.items():
            perf_data.append({
                ...
                'MSE': "{:.4f}".format(metrics['mse']),
                'RMSE': "{:.4f}".format(metrics['rmse']),
                'MAE': "{:.4f}".format(metrics['mae']),
                'R² Score': "{:.4f}".format(metrics['r2'])
            })
        ...
        for model, pred in st.session_state.predictions.items():
            pred_data.append({
                'Model': model.replace('_', ' ').title(),
                'PM2.5 (µg/m³)': "{:.1f}".format(pred['pm25']),
                'Confidence': "{:.0%}".format(pred['confidence']),
                'Trend': pred['trend']
            })
...
def show_multi_city_analysis():
    ...
    for city in st.session_state.selected_cities:
        ...
        comparison_data.append({
            'City': city,
            'PM2.5': "{:.1f}".format(latest['PM2.5']),
            'PM10': "{:.1f}".format(latest['PM10']),
            'NO2': "{:.1f}".format(latest['NO2']),
            'Temperature': "{:.1f}°C".format(latest['Temperature'])
        })
...
def show_advanced_analytics():
    ...
    if st.session_state.data_loaded:
        for city in st.session_state.selected_cities:
            ...
            st.markdown("""
            <div class="alert-box alert-warning">
                <strong>{}: {}</strong> (PM2.5: {:.1f} µg/m³)
            </div>
            """.format(city, status, latest['PM2.5']), unsafe_allow_html=True)
    ...
    if st.button("Generate PDF Report", use_container_width=True):
        with st.spinner("Generating professional PDF report..."):
            try:
                ...
                st.info("Report saved as: {}".format(report_path))
                
            except Exception as e:
                st.error("Error generating report: {}".format(str(e)))
